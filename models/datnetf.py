...
import numpy as np
from models.shares import Base, flip_gradient, CharTDNNHW, BiRNN, CRF, self_attn, focal_loss, Embedding
    ...
    def _build_model(self):
        ...
        with tf.variable_scope("computation_graph"):
            ...
            def compute_src_logits(word_emb, char_emb):
                ...
                rnn_outs = bi_rnn(emb, self.src_seq_len, training=self.is_train)
            ...
            def compute_tgt_logits(word_emb, char_emb):
                char_cnn = char_tdnn_hw(char_emb)
                emb = emb_dropout(tf.concat([word_emb, char_cnn], axis=-1), training=self.is_train)
                rnn_outs = bi_rnn(emb, self.tgt_seq_len, training=self.is_train)
                logits = tgt_dense(rnn_outs)
                transition, mean_loss = tgt_crf_layer(logits, self.tgt_labels, self.tgt_seq_len)
                return rnn_outs, logits, transition, mean_loss

            # train source
            src_rnn_outs, self.src_logits, self.src_transition, self.src_loss = compute_src_logits(
